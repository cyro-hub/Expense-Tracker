...
function Delete({ data }) {
    ...
    return (<>
        <AiFillDelete size={22} onClick={handleModalCloseOrOpen}/>
        <Modal
        open={openForm}
        ...
        <Box sx={style}>
            ...
            <motion.form
              ...
              <motion.div className="input" variants={item}>
                Confirm that you want to <strong className='danger'>delete</strong> {data.name}
              </motion.div>
              <motion.div className="controls" variants={item}>
                <Link to='#' className='danger' onClick={!isLoading && handleSubmit}>
                  {isLoading ? <BeatLoader loading={true} size={8} color="green"/> : "Confirm"}
                </Link>
